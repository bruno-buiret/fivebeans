...
var FiveBeansWorker = function(options)
{
	...
	this.useClientInHandlers = options.useClientInHandlers || false;
...
FiveBeansWorker.prototype.start = function(tubes)
{
	...
	this.client.on('connect', function()
	{
		...
		if(self.useClientInHandlers)
		{
			for(var handler in self.handlers)
			{
				if(self.handlers.hasOwnProperty(handler))
				{
					self.handlers[handler].client = self.client;
				}
			}
		}
